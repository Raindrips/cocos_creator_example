...
export class GoBand {
	...
	public checkWin(): boolean {
		...
		for (let i = 0; i < dir.length; i++) {
			...
			for (let j = 1; j <= 5 && (left || right); j++) {
				...
				if (this.inAear(mx, my) && this.pieceMap[my][mx].color === this.lastPiece.color) {
					fiveCount++;
				}
				else {
					left = false
	...
	public inAear(x: number, y: number) {
		return x >= 0 && x < this.Size && y >= 0 && y < this.Size
	}

	public place(coor: cc.Vec2): boolean {
		this.lastPiece = this.pieceMap[coor.y][coor.x];
